...
y <- round(beta[1] + beta[2]*x + 
             runif(1, 1, 50)*rnorm(length(x)), digits = 4)

# Define UI for application that shows the exercise
ui <- fluidPage(
   
   # Application title
   titlePanel("Quiz: Correlation between y and x"),
   ...
   sidebarLayout(
      ...
      mainPanel(
        h4("Problem."),
        p("Find the correlation between y and x."), 
        p("Report your estimate in the proper box."),
        tableOutput("tablaxy"),
        hr(),
        plotOutput("scatter"),
...
server <- function(input, output, session) {
  ...
  rsubmit <- function(fres, qt) {
    ...
    pre_fill_link <- 
      "https://docs.google.com/forms/d/e/1FAIpQLScvsrwiAYWOOQwPyTDcQheUdtD57p0PlQiGNJhCTMt31cBVhg/viewform?usp=pp_url&entry.1963454833="
    cptr <- "No response"
      ...
      paste("Waiting for your answer ...")
        ...
        if (vans() == 1) {
          qt <- round(difftime(Sys.time(), start_time, units = "mins"), 
                         digits = 6)
          qtime(qt)
          fres <- abs(cor(x,y) - as.numeric(input$corr)) < .001
